...
parser.add_argument('-k', '--keyserver', help='Keyserver hostname without https:// or hkps:// prefix.')
...
if options.refresh:
    subprocess.run(
        ["gpg", "--batch", "--keyserver", f"hkps://{options.keyserver}", "--refresh-keys"],
        check=True
    )
    sys.exit(0)
...
gpg_sha_regex = re.compile(
    r"(?P<hash>\b[0-9a-fA-F]{8}\b).*Fingerprint=(?P<fingerprint>\b[0-9a-fA-F ]+\b)", re.DOTALL
)
